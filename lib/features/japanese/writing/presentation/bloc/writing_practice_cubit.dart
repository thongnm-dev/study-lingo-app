...
import '../../domain/usecases/fetch_writing_characters.dart';
...
class WritingPracticeCubit extends Cubit<WritingPracticeState> {
  WritingPracticeCubit(this._fetchCharacters)
    : super(const WritingPracticeState());

  final FetchWritingCharactersUseCase _fetchCharacters;

  Future<void> load(JapaneseScript script) async {
    final result = await _fetchCharacters(script);
    result.fold(
      (_) => emit(const WritingPracticeState(loaded: true)),
      (characters) =>
